...
def compute_Distance(x1, x2, y1, y2, heightt):
    # horizontal distance between user and uav
    d_h = ((x1 - x2)**2 + (y1 - y2)**2)**0.5
    # 3D distance between user and uav 
    d = ((x1 - x2)**2 + (y1 - y2)**2 + (heightt)**2)**0.5
    return d_h, d


# Calculation of the radius of the coverage area
...
# generate mobility of users randomly 
...
# random selection of slots for user movement
    for i in range(numberIoT):
...
# set initial position of users randomly 
...
# definition of state
        spac = {
            ...
            'RTindex': spaces.Box(low=0, high=1, shape=(10, 1), dtype=np.float64),
...
# forced return to uav initial position 
        if a > b:
