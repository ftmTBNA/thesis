...
def Choose_neighbor(numberIOT, numberChannel, point, initial_pos_UAV):
    number_BE = thr.count(0)
    number_RL = numberIOT - number_BE

    RR = np.zeros((Episode_n, number_BE, numberChannel))
    R_realTime = np.zeros((Episode_n, number_RL, numberChannel))
    np.random.seed(1)
    ...
    while (flag_allocation and iii < 100):
            ...
            for i in range(numberIOT):
                for j in range(numberChannel):
                    dis_h[i], Dis = compute_Distance(
                        position[n][0],  trajectory_X[i][n], position[n][1],  trajectory_Y[i][n], position[n][2])
        ...
        for n in range(Episode_n):
            ...
            R_RL = np.array(rrl)
            R_realTime[n] = R_RL
            sum_r = np.sum(R_realTime, axis=2)
            arrival_R = np.sum(sum_r, axis=0)

            R_BE = np.array(rbe)
            RR[n] = R_BE

            sum_r_BE = np.sum(RR, axis=2)
...
def simulated_annealing(utilitySA, n_iterations, temp, old_position, numberChannel, numberIOT, trajectory_X, trajectory_Y):
    ...
    for i in range(n_iterations):
        ...
        candidate_eval, _, _, _, _, _ = utilitySA(
        ...
        if best_eval == 0 or best_eval == 100:
            stop = 0
...
def utilitySA(point, old_position, numberChannel, numberIOT, trajectory_X, trajectory_Y):
    ...
    R = np.zeros((Episode_n, number_BE, numberChannel))
    R_realTime = np.zeros((Episode_n, number_RL, numberChannel))
    ...
    for n in range(Episode_n):
        ...
        for i in range(numberIOT):
            for j in range(numberChannel):
                dis_h[i], Dis = compute_Distance(
                    position[0],  trajectory_X[i][n], position[1],  trajectory_Y[i][n], position[2])
        ...
        sum_r_BE = np.sum(R, axis=2)
        sum_rbe = np.sum(sum_r_BE, axis=0)
        summmmmmm = np.sum(sum_r_BE, axis=1)
        ...
        if n == 49:
            ...
            for i in range(number_RL):
                ...
                if a < b:
                    ...
                    counterr = counterr + (b-a)
                    flag2 = 1

        cc = counterr

        Fairness = W / (number_BE * Z)
    ...
    if n == 49 and flag == 1 and cc == 0:
        print('OK!')

    sum_U = (SUM) + Fairness * 100 - cc * 30
    Rew = - sum_U

    return Rew, positions, summmmmmm, energy, ff, cc
...
for k in range(len(number_of_channel)):
    ...
    for m in range(len(number_of_IOT)):
        ...
        for counter in range(Counter):
            ...
            state, trajectory_X, trajectory_Y = Uer_Mobilityy(
                Q_X, Q_Y, Episode_n, numberIOT, number_moves, counter)
            global trajectory_X_users, trajectory_Y_users
            trajectory_X_users = trajectory_X
            trajectory_Y_users = trajectory_Y

            best, score, scores = simulated_annealing(
                utilitySA, n_iterations, temp, old_position, numberChannel, numberIOT, trajectory_X, trajectory_Y)
            print(
                '-------------------------------------------------TIME--------------------------------------------')
            ...
            flag = 1
            _, positions, sum_Rate, energy, fairnessSA, cc = utilitySA(
                best, old_position, numberChannel, numberIOT, trajectory_X, trajectory_Y)
            ...
            if (counter+1) % 5 == 0:
                ...
                print(sum(DiffTHR)/(counter+1))
                print(current_time)

                RTmobile.write(" %d\r\n")
                RTmobile.write(" ---number --- "+str(counter+1) +
                               " --------------------- ")
                RTmobile.write(" ---Time --- "+str(current_time) +
                               " --------------------- ")
                RTmobile.write(" ---DiffTHR --- "+str(sum(DiffTHR) /
                                                      (counter+1))+" --------------------- ")
                RTmobile.write(" ---SUM RATE --- " +
                               str(sum(SumRateList)/(counter+1))+" --- ")
                RTmobile.write(" ---FAIRNESS --- " +
